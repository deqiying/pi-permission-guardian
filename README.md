...
> 当前状态：**M6 子代理覆盖已实现**（配置加载/合并/规范化、`/perm` 命令、会话状态与审计日志、tree-sitter 事实提取、glob 规则求值、跨层最严格者合并、会话授权、`tool_call` 与 `user_bash` 端到端裁决、独立评审模型接入与 FR-23 风险门槛、判定缓存/熔断/预评分三项降本机制、子代理会话识别与 `subagentPolicy`）。尚未实现的是分发与端到端验收（M7），见 `docs/implementation-plan.md` 的 M7。
